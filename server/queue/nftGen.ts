...
genNft.process(async (job, done) => {
    const req = job.data;
    const collectionDir = `${basePath}/storage/${req.body.uid}`;
    const buildDir = `${basePath}/storage/${req.body.uid}/build`;
    const layersDir = `${basePath}/storage/${req.body.uid}/layers`;
    let layerOrder = [];

    setupFolder(collectionDir, buildDir, layersDir);
    req.layoutOrder.map(async (item, index) => {
        layerOrder.push({ name: item.name })
        if (fs.existsSync(`${layersDir}/${item.name}`)) {
            fs.rmdirSync(`${layersDir}/${item.name}`, { recursive: true }, (error) => {
        ...
        Promise.all(
            item.listImage.map((element, index) => {
                ...
                fs.writeFileSync(`${layersDir}/${item.name}/${index}.png`, base64Data, 'base64', err => {
                    if (err)
                        console.log('errSaveImage', err);
                    console.log('Saved!');
                });
            })
        )
